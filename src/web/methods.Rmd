...
# Models
We modelled the effects of the program using a linear regression model where the the effects of the program were captured by a variable equal to fixed effects for each year multiplied by an indicator for presence within the project area. After selecting a set of variables we believed could play a role in internet speeds in an area, we ran our model through a step function designed to select only those variables which were most significant.
...
# Conclusion
For each of the three metrics Ookla employs to measure internet quality, the step function recognized the effect of the program as an insignificant factor in determining the overall value of metric. 
